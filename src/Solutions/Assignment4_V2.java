...
public class Assignment4_V2 {
	...
	public static void main(String[] args) {
		...
		int[] evenVector, oddVector;
		int sizeEvenVector, sizeOddVector;
		boolean isFirstEven;

		// Calculate size for each vector
		if (vector.length % 2 == 0) { // even number of elements
			sizeEvenVector = vector.length / 2;
			sizeOddVector = vector.length / 2;
			isFirstEven = (vector[0] % 2 == 0) ? true : false;
		} else { // odd number of elements
			if (vector[0] % 2 == 0) { // first element is even
				sizeEvenVector = vector.length / 2 + 1;
				sizeOddVector = vector.length / 2;
				isFirstEven = true;
			} else { // first element is odd
				sizeEvenVector = vector.length / 2;
				sizeOddVector = vector.length / 2 + 1;
				isFirstEven = false;
			}
		}
		evenVector = new int[sizeEvenVector];
		oddVector = new int[sizeOddVector];
		int start = isFirstEven ? 0 : 1;
		int pos = 0;
		for (int i = start; i < vector.length; i++, i++) {
			evenVector[pos] = vector[i];
			pos++;
		}
		start = isFirstEven ? 1 : 0;
		pos = 0;
		for (int i = start; i < vector.length; i++, i++) {
			oddVector[pos] = vector[i];
			pos++;
		}
		System.out.println("The vector of odd numbers is:   ");
		for (int i = 0; i < oddVector.length; i++) {
			System.out.print(oddVector[i] + " ");
		}
		System.out.println("\nthe vector of even numbers is:   ");
		for (int i = 0; i < evenVector.length; i++) {
			System.out.print(evenVector[i] + " ");
		}
