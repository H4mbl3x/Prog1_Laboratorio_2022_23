		...
		do {
			...
			if(board[r][c] == AVAILABLE) {
				...
				if(availableCount == 0) {
					...
					// set start and end positions to be traversed to count the bombs
					int rStart, rEnd, cStart, cEnd, bombCount = 0;
					rStart = (r-1 >= 0) ? (r-1) : r;
					rEnd = (r+1 < rows) ? (r+1) : r;
					cStart = (c-1 >= 0) ? (c-1) : c;
					cEnd = (c+1 < columns) ? (c+1) : c;
					// traverse the squares, count bombs and show the amount
					for(int i = rStart; i <= rEnd; i++) {
						for(int j = cStart; j <= cEnd; j++) {
							if(board[i][j] == BOMB) bombCount++;
						}
					}
					System.out.println("The selected square is surrounded by " + bombCount + " bombs.");
				}
			} else if(board[r][c] == BOMB) {
				System.out.println("There is a bomb in the selected square.");
				end = true;
			} else { // board[r][c] == MARKED
